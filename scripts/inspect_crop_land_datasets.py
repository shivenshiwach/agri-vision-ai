...
from crop_land_dataset_registry import DEFAULT_REGISTRY_PATH, ReportRow, run_inspection
...
def print_table(rows: list[ReportRow]) -> None:
    ...
    print("---------------------")
    print(f"{'source_dataset':<34} {'original_label':<40} {'normalized_label':<28} {'images':>8} status")
    print("-" * 120)
    for row in sorted(rows, key=lambda item: (item.source_dataset.lower(), item.original_label.lower())):
        source = row.source_dataset[:33]
        original = row.original_label[:39]
        normalized = format_label(row.normalized_label)[:27]
        print(f"{source:<34} {original:<40} {normalized:<28} {row.image_count:>8} {row.status}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect enabled crop/land scene datasets and report normalized labels. Dry-run is the default."
    )
...
def main() -> None:
    ...
    print("Crop/land dataset inspection")
    print("============================")
    print("Mode: dry-run")
    print(f"Registry: {result.registry_path}")
    print(f"Scene taxonomy: {result.taxonomy_path}")
    print(f"Crop taxonomy: {result.crop_taxonomy_path} ({len(result.crop_labels)} crop labels)")
    print("No images were copied. No training was started.")
    print()

    print("Dataset summary")
    print("---------------")
    print(f"{'key':<32} {'status':<22} {'labels':>8} {'mapped':>8} {'unmapped':>9} {'images':>10} root")
    print("-" * 126)
    for inspection in result.datasets:
        mapped = sum(1 for row in inspection.rows if row.status == "mapped")
        unmapped = sum(1 for row in inspection.rows if row.status == "unmapped")
        images = sum(row.image_count for row in inspection.rows)
        print(
            f"{inspection.source_key:<32} {inspection.status:<22} {len(inspection.rows):>8} "
    ...
    print("Missing canonical crop/land labels")
    print("----------------------------------")
